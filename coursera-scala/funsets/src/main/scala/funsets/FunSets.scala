...
object FunSets {
  ...
  def universalOp(baseValue: Boolean, quantifier: (Boolean,Boolean) => Boolean)(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a > bound) baseValue
      else if (contains(s, a)) quantifier(p(a), iter(a+1))
      else iter(a+1)
    }
    iter(-bound)
  }
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = 
    universalOp(true, (b1,b2) => b1 && b2)(s,p)
  ...
  def exists(s: Set, p: Int => Boolean): Boolean = 
    universalOp(false, (b1,b2) => b1 || b2)(s,p)
