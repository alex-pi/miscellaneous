...
object phone {
  ...
  val charCode: Map[Char, Char] =
  	for {
  		(num, letters) <- mnem
  		letter <- letters
  	} yield letter -> num                     //> charCode  : Map[Char,Char] = Map(E -> 3, X -> 9, N -> 6, T -> 8, Y -> 9, J -
	...
	translate("72256944556")                  //> res0: Set[String] = Set(rack my Gil Jo, pack ox Gil Jo, pack my Gil Jo, sac
                                                  //| k ox Gil Jo, sack Oz Gil Jo, rack ox Gil Jo, sack my Gil Jo, rack Oz Gil Jo
                                                  //| , pack Oz Gil Jo)
}