...
abstract class TweetSet {
  ...
  def countRetweets: Int = 
    if(isEmpty) 0
    else head.retweets + tail.countRetweets
...
class Empty extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = if(that.isEmpty) new Empty else that.filter(Tweet => true)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = 
    if(that.isEmpty) new NonEmpty(elem, left, right)
    else (this.tail union that.tail) incl this.head incl that.head
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = groupTweets(TweetReader.allTweets, google)   
  
  val appleTweets: TweetSet = groupTweets(TweetReader.allTweets, apple) 
  
  def groupTweets(tweets: TweetSet, keywords: List[String]): TweetSet = {
    def group(tweets: TweetSet, accu: TweetSet): TweetSet =
	    if(tweets.isEmpty) accu
	    else if(keywords.exists((s: String) => tweets.head.text.contains(s))) group(tweets.tail, accu incl tweets.head) 
	    else group(tweets.tail, accu) 
	group(tweets, new Empty)
  }
...
object Main extends App {
  ...
  println("------------------------")
  println("google:" + GoogleVsApple.googleTweets.countRetweets)
  println("apple:" + GoogleVsApple.appleTweets.countRetweets)
